...
尝试的基本结论是该简单的策略在市场震荡的时候能够保持正向还算不错的收益，但是在市场行情好的时候则表现不佳。本文后面附上的是相关的研究文件和策略回测。本周的笔记主要是解释研究文件中的内容，在这之前先回顾一下@云帆在[高频因子探索——动量交易](https://www.joinquant.com/view/community/detail/22472)使用到的基本思路。
...
如下研究文件里面的代码主要是想按照前两周学习到的思路来重新验证下交易日内的动量效果。如果能够确定交易日开盘价和收盘价之间的大致关系，那么就可以写作策略来进行验证了。比如下面这张图里面是2020-01-01到2020-06-06开盘价和收盘价之间的基本关系：

![](./w24-am-mom.png)

从中大致可以看到当开盘价处在分位数10的时候收盘价的平均涨幅最高，但分位数意味着什么呢？它在这里不是一个绝对值，而是会根据选定的交易日区间的不同而不同。总的来说可以简单概括为“如果开盘的时候涨得好，那么收盘的时候大概率都会涨得好”。

但是，尽管得出了这个结论，在制定策略的时候你必须要经过自己的调试来得到“开盘的时候动量超过什么值时”我才执行买入交易这样具体问题的答案。我在策略里面是制定了“1%”，也就是当开盘第6分钟的价格相对于开盘时上涨超过1%的时候就执行买入，然后第二天卖出，以此来测验。
...
函数`collect_momentums()`会收集多个交易日的动量数据，然后将它们拼接在一起。主要的计算都是在函数`和calculate_momentum()`里面，包括如下步骤：
...
最后按照如上的研究写了一个简单的策略，测试了从2018年至今的收益，还算可以。单纯回测2018年的也还行，但是回测2019年的呢收益就逊色很多啦。所以我前面才提到这种策略对于震荡行情下还是有一些用处。当然，这仅仅是实验，思路和实现上都比较粗陋。
