...
打开一个文件不指定参数，Python默认以读取模式打开文本文件。文件读取的时候可以分行读取，也
可以进行一次读取，但后者对于较大的文件显得乏力。

写文件时需要指定"w"或者"a"参数，前者会覆盖删除上一次打开文件写入的内容，后者表示追加内容。

4.处于二进制文件

如果文件不是文本文件，它就被视为二进制文件，以“b”模式打开，然后就可以访问其各个字节。

文件以"br"模式打开对应二进制读取模式，读取二进制文件时，调用 f.read(n) 来读取接下来的n
个字节。与文本文件对象一样，二进制文件对象也使用文件指针来记录接下来应读取文件的哪个字节。
然而，访问二进制文件是一种非常低级的操作，Python提供了`pickle`，`shelve`以及`sqlite3`
来帮助进行数据存储。
