...
class EtfSelector(object):
    ...
    def get_precheck_list(self):
        ...
        return etf[(etf['start_date'] < dt.date(2019,1,1)) & (etf['end_date'] > dt.date.today())]
    ...
    def select_etf(self):
        ...
        for i in range(len(etf.index)):
            ...
            valuation_di[etf.index[i]] = {'value_quantile':vq, 'time_quantile':tq, 'day_volumn':volumn_day/1000000}

        valuation_df = pd.DataFrame(valuation_di).T.dropna()
        return valuation_df.sort_values(['time_quantile', 'value_quantile'])
...
class IndexValuator(object):
    ...
    def valuate(self, index_code, start_date, end_date):
        '''计算指数的估值数据。
    ...
    def get_valuation(self, index_code, start_date):
        ...
        df_new = self.valuate(index_code, start_date=start_date, end_date=self._end_date)
        Utility.save_to_db(file_name, new=df_new, old=df_old)
...
class HtmlReporter(object):
    def __init__(self, date):
        ...
        self._index_valuator = IndexValuator(date)
    ...
    def construct_ETF_list(self):
        ...
        <article>
          <p>注1：当前根据经验将数值百分位和时间百分位均小于60%作为“运行在历史低位”的判断标准，均大于80%作为“运行在历史在高位”的判断标准。对于该基金是否高估还是低估需要进一步从基金对应的指数来进行判断。</p>
          <p>注2：下面的图示展示的是正在定投的ETF对应指数的估值状态。</p>
        </article>
        </div>
        <section> </section>
        """
    ...
    def send_email(self, sender, receiver):
        ...
        # 填充邮件头部
        msg['Subject'] = 'ETF投资参考日报 - ' + str(self._date)
        msg['From'] = sender
        msg['To'] = receiver

        # 填充邮件正文
        indexes = ['000905.XSHG', '000068.XSHG', '000952.XSHG', '399975.XSHE']
        html = self._head \
