...
def get_index_pe(index=None):
    ...
    index_pe = {item : get_pe_pb(item, get_security_info(item).start_date) for item in index if is_only_stock(item) == True}
    print("获取数据完成。")
...
def draw_quantile_diagram(index, indicator, years, valuation):
    ...
    i_df['history'] = df[indicator].rolling(years * 244).apply(lambda x: H_V_H(x), raw=True)

    # 三、计算当前（最近一个交易日）的近years年的百分位高度，用于估值
    quantile_now = H_V_H(i_df[indicator])
    assessment = get_valuation_status(quantile_now)
    title = '{}，{} {}={}，近{}年历史百分位{}，{}'.format(get_security_info(index).display_name,
                                               valuation.index[-1],
